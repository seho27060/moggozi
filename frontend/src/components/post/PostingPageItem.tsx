  ...
  return(
    <div className={styles.postItem}>
      <div className={styles.header}>
        ...
        <div style={{display: 'flex', cursor: 'pointer'}} onClick={() => {navigate(`/user/${post.writer?.id}`)}}>
        ...
        <div className={styles.shared} onClick={urlCopy}><ShareIcon /></div>
      </div>

      {post.postImg ? <img onClick={(event: React.MouseEvent) => {
                    event.preventDefault();
                    dispatch(setModalPostState(post));
                    dispatch(setPostModalOpen(true));
                  }} className={styles.postImg} src={post.postImg!} alt="" /> : <img className={styles.postImg} src="" alt="" /> }
