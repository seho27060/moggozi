    ...
    <div className={styles.container}>
      <div className={styles.writer}>
        <div className={styles.profile}>
          <img src={child.writer?.path} alt="" />
          {/* <p>{child.order}</p> */}
          <div>{child.writer?.nickname}</div>
        </div>
        <div className={styles.option}><CommentOptionBtn comment={child} postId = {postId}/></div>
      </div>
      <div>
        <div className={styles.comment}>{child.text}</div>
        <div className={styles.date}>{child.modifiedTime?.toString().slice(0, 4)}년 {child.modifiedTime?.toString().slice(6, 7)}월 {child.modifiedTime?.toString().slice(8, 10)}일</div>
      </div>
    </div>
  );
};
